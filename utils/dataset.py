...
class Dataset(data.Dataset):
    ...
    def load_label(filenames):
        ...
        for filename in filenames:
            label_path = filename.replace("images", "labels").replace(".png", ".txt")
            ...
            assert labels.shape[1] == 5, "labels require 5 columns"
            assert (labels >= 0).all(), "negative label values"
            assert len(labels) > 0, "no labels found"

            x[filename] = (labels, shape)
